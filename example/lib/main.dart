...
class _MyAppState extends State<MyApp> {
  ...
  void _createClient() async {
    _client =
        await AgoraRtmClient.createInstance('03e7394bd8b3429bbeeb4e1356c40d72');
    ...
    _client.onLocalInvitationAccepted = (AgoraRtmLocalInvitation invite){
      ...
      _log('onLocalInvitationAccepted: ' +invite.toString());

    };

    _client.onLocalInvitationCanceled = (AgoraRtmLocalInvitation invite){
      _log('onLocalInvitationCanceled: ' +invite.toString());
    };

    _client.onLocalInvitationFailure = (AgoraRtmLocalInvitation invite, int errorCode){
      _log('onLocalInvitationFailure: ' +invite.toString());
    };


    _client.onRemoteInvitationAccepted = (AgoraRtmRemoteInvitation invite){
      _log('onRemoteInvitationAccepted: ' +invite.toString());
    };

    _client.onRemoteInvitationCanceled = (AgoraRtmRemoteInvitation invite){
      _log('onRemoteInvitationCanceled: ' +invite.toString());
    };
    _client.onRemoteInvitationFailure = (AgoraRtmRemoteInvitation invite ,int errorCode){
      _log('onRemoteInvitationFailure: ' +invite.toString());
    };
    ...
    return Row(children: <Widget>[
      new Expanded(
          child: new TextField(
              ...
              decoration: InputDecoration(hintText: 'Input callee Id'))),
      new OutlineButton(
  ...
  void call(){
    ...
    Map<dynamic,dynamic> mp = {"channelId":_channel.channelId,
      "content":"testing","calleeId":_callController.text,
      "state":_state,"response":""};

    print(mp.toString());

    _client.sendLocalInvitation(mp);
  }

  void accept(){

    _client.acceptRemoteInvitation(null);
  }
}
